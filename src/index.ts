...
export function getFormattedSrcLink(filepath: string, repoUrl?: RepoUrl) {
	...
	if (typeof repoUrl === "string") {
		srcLink = `[${filepath}](${repoUrl}/blob/${
			danger.git.commits.slice(-1)[0].sha
		}/${filepath})`
...
export function prepareTodosForDanger(
	keywords: string[] | undefined,
	addedText: string,
	removedText: string,
	filepath: string,
	repoUrl: RepoUrl,
	keywordMatches: PlainObject<string[]>,
...
export default async function todos({
	...
	const mergedKeywordMatches: PlainObject<string[]> = merge(
		keywordMatches,
		...results,
	)
